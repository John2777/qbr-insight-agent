...
def test_term_question_without_documents_uses_evidence_boundary_not_curated_answer_template(tmp_path: Path) -> None:
    ...
    assert run["message"]["metadata"]["pipeline_version"] == "semantic-task-frame-v2-typed-coverage"
    assert "intent" not in run["message"]["metadata"]["query_plan"]
    assert history["messages"][-1]["metadata"]["pipeline_version"] == "semantic-task-frame-v2-typed-coverage"
