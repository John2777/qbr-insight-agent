...
class EmbeddingProvider(Protocol):
    """Define the contract for converting text into embedding vectors."""
    model: str
    identity: str
    ...
    def embed(self, texts: Sequence[str]) -> list[list[float]]:
        """Convert the supplied texts into normalized embedding vectors."""
        ...


class VectorSearchBackend(Protocol):
    """Define the contract for indexing and searching vector evidence."""
    backend_name: str
    ...
    def index_document_version(self, workspace_id: str, document_version_id: str) -> None:
        """Index active chunks for one document version."""
        ...

    def rebuild_workspace(self, workspace_id: str) -> None:
        """Rebuild all vector evidence for one workspace."""
        ...
...
class OpenAICompatibleEmbeddingProvider:
    ...
    def __init__(self, settings: Settings) -> None:
        """Initialize an OpenAI-compatible embedding client from settings."""
...
class HashingEmbeddingProvider:
    ...
    def __init__(self, dimensions: int = 384) -> None:
        """Initialize the hashing embedding provider and its dependencies."""
    ...
    def _features(text: str) -> list[str]:
        """Extract features for this hashing embedding provider."""
...
class FaissVectorStore:
    ...
    def __init__(self, db: Database, settings: Settings, provider: EmbeddingProvider) -> None:
        """Initialize the faiss vector store and its dependencies."""
        try:
    ...
    def _stem(self, workspace_id: str) -> str:
        """Return the stable filesystem stem for a workspace index."""
    ...
    def _index_path(self, workspace_id: str) -> Path:
        """Return a workspace-specific vector index or manifest path."""
    ...
    def _active_chunks(self, workspace_id: str) -> list[dict[str, Any]]:
        """Return active or missing chunks required to synchronize the index."""
    ...
    def index_workspace(self, workspace_id: str) -> None:
        """Synchronize the vector index for one workspace."""
        with self._lock:
    ...
    def _embedding_rows(self, workspace_id: str) -> list[dict[str, Any]]:
        """Return embedding rows in deterministic chunk order."""
        with self.db.read() as conn:
            rows = conn.execute(
                """
    ...
    def _signature(rows: Sequence[dict[str, Any]]) -> str:
        """Build a stable signature for the active embedding corpus."""
    ...
    def _manifest_is_current(self, workspace_id: str) -> bool:
        """Return whether the on-disk index manifest matches active evidence."""
    ...
    def _normalize(self, matrix: Any) -> Any:
        """Normalize a vector to unit length for cosine-compatible search."""
    ...
    def _load_index(self, workspace_id: str) -> Any | None:
        """Load a validated FAISS index and its manifest."""
