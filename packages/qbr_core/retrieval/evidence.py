...
from packages.qbr_core.retrieval.coverage import CoverageResult, build_evidence_contract, evaluate_evidence_coverage, facet_ids_for_evidence
...
class EvidenceAtom:
    """Represent one self-contained claim source with stable provenance."""
    atom_id: str
    quote: str
    content_role: str
    facet: str
    ...
    def to_evidence(self) -> dict[str, Any]:
        """Convert this atom into the public evidence dictionary."""
        row = self.source
        return {
...
@dataclass(frozen=True, slots=True)
class EvidencePack:
    """Group bounded evidence atoms and their coverage diagnostics."""
    ...
    def evidence(self) -> list[dict[str, Any]]:
        """Return public evidence dictionaries for all atoms."""
        return [atom.to_evidence() for atom in self.atoms]

    def render_grounding_context(self, plan: QueryPlan) -> str:
        """Render bounded model grounding context with stable evidence identifiers."""
        if not self.atoms:
            return (
                "没有检索到可用于回答的文档证据。"
    ...
    def render_fallback(self, plan: QueryPlan) -> str:
        """Render a safe evidence-only fallback answer."""
...
class EvidencePackBuilder:
    ...
    def build(
        ...
        *,
        ...
        """Build the domain result from the supplied inputs."""
