...
class RerankScore:
    """Associate a retrieval candidate index with its relevance score."""
    index: int
    relevance_score: float


class Reranker(Protocol):
    """Define the contract for scoring retrieved evidence candidates."""
    model: str

    def rerank(self, query: str, documents: Sequence[str], *, top_n: int) -> list[RerankScore]:
        """Score and reorder evidence candidates by relevance."""
        ...
...
class QwenReranker:
    ...
    def __init__(self, settings: Settings) -> None:
        """Initialize the qwen reranker and its dependencies."""
    ...
    def _endpoint(base_url: str) -> str:
        """Build the provider-specific rerank endpoint URL."""
