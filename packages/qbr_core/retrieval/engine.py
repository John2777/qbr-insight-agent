...
class RetrievalResult:
    """Carry ranked retrieval sources together with pipeline diagnostics."""
...
class EvidenceRetriever:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the evidence retriever and its dependencies."""
        self.db = db
        self.mode = mode
    ...
    def vector_available(self) -> bool:
        """Return the vector available for this evidence retriever."""
        return self.vector_store is not None

    @property
    def vector_backend(self) -> str | None:
        """Return the vector backend for this evidence retriever."""
        return self.vector_store.backend_name if self.vector_store else None

    @property
    def rerank_available(self) -> bool:
        """Return the rerank available for this evidence retriever."""
    ...
    def _hydrate_source_metadata(row: dict[str, Any]) -> dict[str, Any]:
        """Enrich source metadata for this evidence retriever."""
        try:
    ...
    def _task_compatibility(plan: QueryPlan, role: str, content: str) -> float:
        """Score whether a source can support the planned analytical operation."""
        role_bonus = {
            "business_fact": 0.4,
    ...
    def _lexical_search(
        ...
        *,
        ...
        """Run the bounded lexical evidence search."""
    ...
    def _scope(document_ids: list[str]) -> tuple[str, list[Any]]:
        """Build the SQL scope predicate for workspace and document filters."""
        if not document_ids:
            return "", []
