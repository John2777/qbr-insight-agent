    ...
    def render_explanation(self, plan: QueryPlan) -> tuple[str, list[dict[str, Any]], list[str]]:
        ...
        if not selected_signals and not selected_status:
            answer = (
                ...
                if language == "zh"
                else "The document does not provide business indicators, risk gates, or management actions sufficient to define this risk concept."
            )
        ...
        if language == "zh":
            definition = (
                ...
                if any(marker in concept.casefold() for marker in ("执行", "落地", "交付", "实施", "execution", "delivery", "implementation"))
            ...
            if selected_signals:
                ...
                lines = [
                    f"- **{labels[signal.category]}**：{signal.claim('zh')} [{citation(signal.evidence)}]"
            ...
            definition = (
                "the possibility that agreed business goals or priorities deviate from plan during delivery, or trigger a management threshold"
                if any(marker in concept for marker in ("execution", "delivery", "implementation"))
                else "the possibility that the relevant business condition deviates from expectation, triggers a management threshold, and affects delivery of the plan"
            )
            sections = [
                "## Direct explanation\n\n"
                f"The document does not provide a formal glossary definition of {concept}. From its risk gates, KPIs, and management actions, "
                f"the term can be understood as **{definition}**. This is an evidence-based interpretation, not a quoted definition. {basis_refs}",
            ]
            if selected_signals:
                labels = {
                ...
                lines = [
                    f"- **{labels[signal.category]}**: {signal.claim('en')} [{citation(signal.evidence)}]"
                    for signal in selected_signals
                ]
            ...
            if selected_status:
                ...
                sections.append(
                    ...
                    "The current values and green/amber/red gates are the quantitative test: an amber/red result or sustained deterioration "
                    f"supports escalation, while a green status does not prove that future execution risk is absent. {refs}"
                )
            sections.append(
                "## Boundary\n\n"
                f"{concept.capitalize()} is uncertainty around delivery; it is not the same as a loss that has already occurred. "
                "Only verifiable metric changes, thresholds, and management actions from the document are treated as concrete manifestations."
            )
