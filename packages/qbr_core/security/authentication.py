...
from packages.qbr_core.foundation.errors import AuthenticationRequired, PermissionDenied, TooManyRequests
...
class AuthPrincipal:
    """Identify an authenticated user, workspace, and authorization scopes."""
    workspace_id: str
    user_id: str
    roles: tuple[str, ...]

    def require(self, permission: str) -> None:
        """Require the named authorization scope for this principal."""
...
class LoginRateLimiter:
    ...
    def __init__(self, max_attempts: int, window_seconds: int) -> None:
        """Initialize the login rate limiter and its dependencies."""
    ...
    def check(self, key: str) -> None:
        """Reject a login key while its failure window is rate limited."""
        now = time.monotonic()
        with self._lock:
    ...
    def failure(self, key: str) -> None:
        """Record a failed login attempt for the supplied key."""
        with self._lock:
